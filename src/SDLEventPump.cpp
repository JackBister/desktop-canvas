  ...
  while (SDL_PollEvent(&e)) {
    ...
    if (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP) {
      ...
      if (codeOptional.has_value()) {
        JSObject keyboardEvent = {{"altKey", (key.keysym.mod & KMOD_ALT) != 0},
      ...
      JSObject touch = {
          {"clientX", (double)clientX}, {"clientY", (double)clientY}, {"force", finger.pressure}};

      JSArray changedTouches = {touch};

      JSObject touchEvent = {{"type", getTouchEventType(e.type)},
