...
{
  auto window = SDL_CreateWindow("SDL", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 256, 144,
  ...
  if (!renderer) {
    __android_log_print(ANDROID_LOG_INFO, "dcanvas", "renderer NULL! %s, trying CreateRenderer...",
                        SDL_GetError());
    ...
    if (!renderer) {
      ...
      if (!renderer) {
        __android_log_print(ANDROID_LOG_INFO, "dcanvas", "renderer still NULL! %s, giving up...",
                            SDL_GetError());
        return 1;
      }
    }
