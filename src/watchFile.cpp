...
struct CallbackParam {
    ...
    CallbackParam(HANDLE watchHandle, std::function<void()> callback)
        : watchHandle(watchHandle), callback(callback)
    {
    }
};
...
void watchFile_W32(std::wstring filename, std::function<void()> onChange)
{
    ...
    auto watchHandle =
        FindFirstChangeNotification(absolutePath.c_str(), false, FILE_NOTIFY_CHANGE_LAST_WRITE);
    HANDLE newWaitObject;

    void * callbackArgs = new CallbackParam(watchHandle, onChange);
    auto registerResult =
        RegisterWaitForSingleObject(&newWaitObject, watchHandle, watch_file_callback, callbackArgs,
