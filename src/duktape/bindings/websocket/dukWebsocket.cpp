...
static SafeQueue<std::pair<std::function<void(duk_context *, MessageEvent)>, MessageEvent>> *
    webSocketMessageQueue = nullptr;
...
static int setOnMessage(duk_context * ctx)
{
  ...
  ws->setOnMessage([ws](MessageEvent msg) {
    webSocketMessageQueue->push(std::make_pair(
