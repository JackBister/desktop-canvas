...
static int addEventListener(duk_context * ctx)
{
    ...
    if (duk_is_undefined(ctx, -1)) {
        ...
        duk_put_prop_string(ctx, -2,
                            eventName.c_str()); // eventName, fn, globalStash, __eventHandlers
        duk_get_prop_string(
            ctx, -1,
            eventName.c_str()); // eventName, fn, globalStash, __eventHandlers, array of handlers
    }
    auto numHandlers = duk_get_length(ctx, -1);
    duk_swap_top(ctx, -4); // eventName, array of handlers, globalStash, __eventHandlers, fn
    duk_require_function(ctx, -1);
    duk_put_prop_index(ctx, -4,
                       numHandlers); // eventName, array of handlers, globalStash, __eventHandlers
    duk_pop_n(ctx, 4);
    return 0;
}
...
static int removeEventListener(duk_context * ctx)
{
    ...
    for (auto i = 0; i < numHandlers; ++i) {
        duk_get_prop_index(
            ctx, -1, i); // eventName, fn, globalStash, __eventHandlers, array of handlers, handler
        duk_require_function(ctx, -1);
...
void dcanvas::callEventListeners(duk_context * ctx, std::string const & eventName, JSValue evt)
{
    ...
    for (auto i = 0; i < numHandlers; ++i) {
        ...
        dcanvas::dukUtils::pushToCtx(
            ctx, evt); // globalStash, __eventHandlers, array of handlers, handler, evt
        auto result =
            duk_pcall(ctx, 1); // globalStash, __eventHandlers, array of handlers, (ret or err)
        if (result != 0) {
            duk_safe_to_string(ctx, -1);
            logger->info("Got error when handling eventName=%s, error=%s", eventName.c_str(),
