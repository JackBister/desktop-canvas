...
SDLNavigator::SDLNavigator()
{
    ...
    for (int i = 0; i < SDL_NumJoysticks(); ++i) {
        if (SDL_IsGameController(i)) {
            ...
            if (!gameController) {
                logger->info("ERROR: Joystick idx %d was identified as a gamepad but "
                             "SDL_GameControllerOpen failed",
                             i);
            }
...
int SDLNavigator::sdlEventWatch(void * userdata, SDL_Event * evt)
{
    ...
    if (evt->type == SDL_CONTROLLERDEVICEADDED) {
        ...
        auto removeMe = std::find_if(
            nav->gamepads.begin(), nav->gamepads.end(),
            [instanceId](SDLGamepad & gamepad) { return gamepad.getJoystickId() == instanceId; });
